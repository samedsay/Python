...
for item in daily_transactions_split:
  newItem = []
  for line in item:
    newItem.append(line.strip())
  transactions_clean.append(newItem)
...
for sale in sales:
  priceStr = sale.strip('$')
  newSales.append(priceStr)
  total_sales += float(priceStr)
print(newSales)
print(total_sales)
...
for color in thread_sold:
  if color.find("&"):
    ...
    for newCol in newColor:
      thread_sold_split.append(newCol)
  else:
...
newThread = []
for thread in thread_sold_split:
  if thread not in newThread:
    newThread.append(thread)
print(newThread)

newThread = set(thread_sold_split)
colors = list(newThread)
print(colors)

for item in colors:
  print("Number of {} is {}.".format(item,color_count(item)))
